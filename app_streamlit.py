# -*- coding: utf-8 -*-
"""Agentic_AI_(Gradio).ipynb

Automatically generated by Colab.

Original file is located at
    https://colab.research.google.com/drive/1GVEYNtZ8_Gq5bPiKmY_rE8TSZ2iFwuA8
"""
"""
...
"""
# enhanced_crypto_analyst_single_send.py
"""
Enhanced Crypto Analyst - single-send Gradio UI.

Features:
- Coingecko market, Prophet, LSTM, FinBERT sentiment
- Short-term memory (SQLite)
- Long-term memory (embeddings + FAISS) - optional if packages installed
- Continual retraining skeleton (manual function)
- Planner stub
- Explainability trace (weights, components, top headlines)
- Gradio UI: one Textbox + one Send button → returns summary, explainability, headlines, chart
"""
...
# Hugging Face token (optional; set env HF_TOKEN if you have one)
...
def recommend_and_insight(sentiment: float, pct_24h: float, pct_7d: float, rsi: float, risk: str, horizon_days: int) -> Dict:
    ...
    # Build the insight with a clearer structure and actionable language
    pieces = []
...
def generate_risk_disclosure(result: Dict) -> str:
    """
    Build a dynamic Risk Disclosure section using model outputs.
    Uses: pct_change_24h, pct_change_7d, rsi_14, sentiment percentages,
          market_cap, volume_24h (if present), headlines, Prophet summary (if present).
    """
    market = result.get("market", {})
    pct_24h = market.get("pct_change_24h")
    pct_7d = market.get("pct_change_7d")
    rsi = market.get("rsi_14")

    # Optional fields (may be missing depending on your analyze_coin)
    ...
    prophet_summary = result.get("prophet_summary", {}) or {}
    articles = result.get("articles", []) or []

    bullets = []

    # 1) Volatility risk (price swings)
    try:
        if pct_24h is not None and not (isinstance(pct_24h, float) and math.isnan(pct_24h)):
            if abs(pct_24h) >= 5:
                bullets.append(f"- **Volatility Risk**: 24h move {pct_24h:+.2f}% indicates elevated short-term swings.")
        if pct_7d is not None and not (isinstance(pct_7d, float) and math.isnan(pct_7d)):
            if abs(pct_7d) >= 12:
                bullets.append(f"- **Trend Volatility**: 7d move {pct_7d:+.2f}% suggests larger trend risk.")
    except Exception:
        pass

    # 2) Technical/RSI risk
    try:
        if rsi is not None and not (isinstance(rsi, float) and math.isnan(rsi)):
            if rsi >= 70:
                bullets.append("- **Overbought Risk (RSI ≥ 70)**: Higher probability of pullback.")
            elif rsi <= 30:
                bullets.append("- **Oversold Risk (RSI ≤ 30)**: Elevated whipsaw risk despite bounce potential.")
    except Exception:
        pass

    # 3) Momentum breakdown / downtrend
    try:
        if pct_7d is not None and not (isinstance(pct_7d, float) and math.isnan(pct_7d)) and pct_7d <= -10:
            bullets.append("- **Downtrend Risk**: 7-day drawdown exceeds −10%; further downside possible if support breaks.")
    except Exception:
        pass

    # 4) Liquidity risk (Volume vs Market Cap) — only if both present
    try:
        if (mcap is not None and vol24 is not None
            and not (isinstance(mcap, float) and math.isnan(mcap))
            and not (isinstance(vol24, float) and math.isnan(vol24))
            and mcap > 0):
            vol_ratio = vol24 / mcap
            # Heuristic: < 2–3% can indicate thinner liquidity for smaller caps
            if vol_ratio < 0.02:
                bullets.append(f"- **Liquidity Risk**: 24h volume is {vol_ratio:.2%} of market cap (thin order book risk).")
    except Exception:
        pass

    # 5) Newsflow / sentiment risk
    try:
        if (neg_pct - pos_pct) >= 20:
            bullets.append("- **Negative Newsflow Risk**: Negative coverage materially outweighs positive.")
    except Exception:
        pass

    # 6) Regulatory/operational risk from headlines (keyword heuristic)
    try:
        headline_texts = " ".join([a.get("title", "") + " " + a.get("summary", "") for a in articles]).lower()
        reg_keywords = ["sec", "cftc", "etf", "lawsuit", "ban", "probe", "investigation", "regulator", "compliance"]
        op_keywords  = ["hack", "exploit", "outage", "downtime", "breach"]
        if any(k in headline_texts for k in reg_keywords):
            bullets.append("- **Regulatory Risk**: Recent headlines reference regulatory actions or reviews.")
        if any(k in headline_texts for k in op_keywords):
            bullets.append("- **Operational Risk**: Reports of hacks/outages can impact confidence and price.")
    except Exception:
        pass

    # 7) Forecast uncertainty (Prophet interval width if available)
    try:
        pred_last = prophet_summary.get("pred_last")
        upper = prophet_summary.get("upper")
        lower = prophet_summary.get("lower")
        if all(v is not None for v in [pred_last, upper, lower]):
            span = abs(upper - lower)
            if pred_last and not (isinstance(pred_last, float) and math.isnan(pred_last)) and pred_last != 0:
                rel_span = span / abs(pred_last)
                if rel_span >= 0.30:
                    bullets.append("- **Forecast Uncertainty**: Wide prediction interval (≥30% of point estimate).")
    except Exception:
        pass

    # Fallback generic risk if nothing triggered
    if not bullets:
        bullets.append("- **General Risks**: Crypto assets are volatile and subject to liquidity, regulatory, and macro uncertainties.")
...
def _pick_insight_line(insight_text: str, label: str, fallback: str = "—") -> str:
    """
    Returns the full line from insight_text that begins with the given label.
    Handles variants with/without markdown bold and optional colon.
    """
    if not insight_text:
        return fallback
    # normalize into lines
    lines = [ln.strip() for ln in insight_text.splitlines() if ln.strip()]
    # Build patterns to match "**Label**:" or "Label:" or "Label"
    patterns = [
        rf"^\*\*{re.escape(label)}\*\*\s*:?\s*(.+)$",  # **Label**: ...
        rf"^{re.escape(label)}\s*:?\s*(.+)$",          # Label: ...
    ]
    for ln in lines:
        for pat in patterns:
            ...
            if m:
                # return the whole original line for fidelity
                return ln
    return fallback
...
def make_pretty_output(result: Dict, horizon_days: int) -> str:
    ...
    # Market cap & volume (assumes analyze_coin sets these; else falls back to NA)
    ...
    # --- Risk Disclosure (with explainability, no duplication elsewhere)
    ...
    # Regulatory signal from headlines
    try:
    ...
    # Explainability metrics inside Risk Disclosure
    ...
    # --- Use the full sentences from the insight for Momentum & RSI Analysis
...
def build_single_response(user_message: str, session_id: str):
    ...
    # Build pretty outputs
    pretty = make_pretty_output(result, h)

    # Friendly explainability summary (exam-style 0–100 score)
    ex = result.get("explainability", {})
    comps = ex.get("components", {})
    ...
    # 2. Explain each component nicely
    ...
    # Chart: create image file and read into bytes (gr.Image accepts filepath)
    ...
    # Save assistant reply and explain trace to memory
...
# ---------------------------
# Gradio UI: single send button
# ---------------------------
with gr.Blocks(title="Crypto Analyst — Single Send") as demo:
    gr.Markdown("""
    # 💬 Crypto Analyst (Single Send)
    Ask about any supported crypto (BTC, ETH, SOL, etc).
    Type your question and click **Send** — you'll get a summary, an explainability summary + full trace, top headlines, and a forecast chart.
    ⚠️ Educational only — not financial advice.
    """)

    chat_history = gr.Chatbot(label="Conversation", height=420)
    user_input = gr.Textbox(placeholder="Type your question (e.g. 'Should I buy Bitcoin?')", lines=2, label="Your message")
    ...
    with gr.Tab("🔎 Explainability"):
        explain_friendly = gr.Textbox(label="Friendly Explainability", lines=6, interactive=False)
        explain_full = gr.JSON(label="Full Technical Trace")

    with gr.Tab("📰 Headlines"):
        headlines_out = gr.Textbox(label="Top Headlines (with sentiment)", lines=8, interactive=False)

    with gr.Tab("📈 Forecast Chart"):
        chart_out = gr.Image(type="filepath", label="Price & Forecast Chart")

    # session state
    session_state = gr.State(str(uuid.uuid4()))

    def on_send(user_message, chat_history, session_id):
        chat_history = chat_history or []
        pretty_text, full_ex, headlines_text, chart_path = build_single_response(user_message, session_id)
        # append to chat view: keep (user, assistant) tuple
        chat_history.append((user_message, pretty_text))
        # update tab outputs
        friendly = ""
        if full_ex:
            comps = full_ex.get("components", {})
            ...
            if total is not None:
                if total > 0.05:
                    ...
                    mood = "neutral 😐"
                friendly = f"Model score: {total:.3f} → {mood}\n"
            for k in ["rsi_component", "sentiment_component", "mom24_component", "mom7_component"]:
                if k in comps:
                    friendly += f"{k.replace('_',' ').capitalize()}: {comps[k]:+.4f}\n"
        return chat_history, pretty_text, full_ex or {}, headlines_text, chart_path, session_id

    send_btn.click(on_send, inputs=[user_input, chat_history, session_state], outputs=[chat_history, summary_out, explain_full, headlines_out, chart_out, session_state])

if __name__ == "__main__":
    demo# Removed Gradio launch for Streamlit
